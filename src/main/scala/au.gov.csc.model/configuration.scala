...
object Configuration {
  ...
  def configure = {
    val filePath = "appConf/configuration.xml" // get this from a systemVariable
    val xml = scala.xml.XML.load(filePath)

    println("configuring from: %s".format(xml))

    Globals.tokenGenerator = getChildElems((xml \\ "tokenProvider")) match {
      case List(smtp: Elem) if smtp.label == "stableMockTokenProvider" => {
        new StableMockTokenGenerator((smtp \\ "@token").headOption.map(_.text).getOrElse({ throw new Exception("no token provided on stableMockTokenProvider") }))
      }
    ...
    Globals.tokenSender = getChildElems((xml \\ "tokenSender")) match {
    ...
    Globals.userProvider = getChildElems((xml \\ "factProvider")) match {
          ...
          for {
            person <- (m \\ "person").headOption.flatMap(p => {
              for {
                ...
                Person(sn, fn, dob, age, fullName, gender, (p \\ "@title").headOption.map(_.text), (p \\ "@tfn").headOption.map(_.text))
              }
            })
